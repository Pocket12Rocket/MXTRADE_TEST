...
export async function fetchUserOrders(email) {
  ...
  const snapshot = await getDocs(ordersQuery);
  // Flatten all items in each order for display (one row per product)
  const orders = [];
...
export async function submitProductRequest({ user, name, price, category, subcategory, description, specifications, files, customFields = {} }) {
  try {
    ...
    if (error?.code === 'storage/unauthorized') {
      throw new Error('Image upload blocked by Firebase Storage Rules. Allow authenticated write access for testing.');
    }
...
export async function fetchLiveProducts(options = {}) {
  ...
  // Public views should only show listed products, but admin can include all lifecycle states.
  const productsQuery = includeAllStatuses
    ? query(collection(db, 'products'), orderBy('createdAt', 'desc'))
    : query(collection(db, 'products'), where('status', '==', 'listed'), orderBy('createdAt', 'desc'));
  const snapshot = await getDocs(productsQuery);
  const products = snapshot.docs.map((docItem) => normalizeProductRecord({ id: docItem.id, ...docItem.data() }));
...
function normalizeProductRecord(record) {
  ...
  const normalizedStatus = record.status === 'paid'
    ? 'purchased'
    : (record.status || (record.marketSold ? 'purchased' : 'listed'));
...
export async function createOrder({ buyerId, buyerEmail, items, totalAmount, shippingAddress }) {
...
export async function rejectSubmission(submissionId, adminId, reason) {
  ...
  await updateDoc(submissionDoc, {
    ...
    rejectedBy: adminId,
    rejectionReason: reason || 'Rejected by admin at this time.',
  });
}
