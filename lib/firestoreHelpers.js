...
export async function processRefundRequest({ orderId, action, adminResponse }) {
  ...
  await runTransaction(db, async (transaction) => {
    ...
    if (!currentRefund.exists() || !orderSnapshot.exists()) {
      throw new Error('Refund request or order not found.');
    }
    ...
    if (refundAccepted) {
      productSnapshots.forEach((productSnapshot, index) => {
        if (!productSnapshot.exists()) {
          throw new Error('A product from this order could not be restocked.');
        }
    ...
    transaction.update(orderRef, {
      status: refundAccepted ? 'refunded' : 'delivered',
...
export async function updateAboutContent(content, adminId = 'admin') {
  ...
  if (!aboutUsBody || !howItWorksBody) {
    throw new Error('About us content and How it works content are required.');
  }
...
import { collection, addDoc, doc, getDoc, getDocs, query, where, orderBy, updateDoc, deleteDoc, serverTimestamp, setDoc, increment, runTransaction } from 'firebase/firestore';
...
export async function submitProductRequest({ user, name, price, category, subcategory, description, specifications, quantity = 1, files, customFields = {} }) {
  try {
    ...
    if (normalizedDescription.length > MAX_LISTING_DESCRIPTION_LENGTH) {
      throw new Error(`Description must be ${MAX_LISTING_DESCRIPTION_LENGTH} characters or fewer.`);
    }
...
export async function fetchLiveProducts(options = {}) {
  ...
  if (includeAllStatuses) {
    ...
    products = snapshot.docs.map((docItem) => normalizeProductRecord({ id: docItem.id, ...docItem.data() }));
  } else {
    const [listedSnapshot, activeSnapshot] = await Promise.all([
      getDocs(query(
    ...
    products = Array.from(new Map(
      [...listedSnapshot.docs, ...activeSnapshot.docs]
        .map((docItem) => {
          const normalized = normalizeProductRecord({ id: docItem.id, ...docItem.data() });
  ...
  const productsWithSellerInfo = await Promise.all(
    products.map(async (product) => {
      if (product.sellerId) {
        try {
          const sellerProfile = await fetchSellerPublicProfile(product.sellerId);
          return {
            ...product,
            sellerSuburb: sellerProfile?.suburb || product.sellerSuburb || '',
            sellerCity: sellerProfile?.city || product.sellerCity || '',
            sellerBadge: sellerProfile?.sellerBadge || product.sellerBadge || '',
            sellerTrustScore: sellerProfile?.sellerTrustScore ?? product.sellerTrustScore ?? null,
          };
        } catch {
          return {
            ...product,
            sellerSuburb: product.sellerSuburb || '',
            sellerCity: product.sellerCity || '',
            sellerBadge: product.sellerBadge || '',
            sellerTrustScore: product.sellerTrustScore ?? null,
          };
        }
      }
      return {
...
export async function upsertSellerPrivateProfile(user, sellerData = {}) {
  ...
  const withPermissionContext = (error, step) => {
    if (error?.code === 'permission-denied') {
      return new Error(`Missing or insufficient permissions while saving ${step}.`);
    }
    return error;
  };
  ...
  const privateProfileSnapshot = await getDoc(privateProfileRef).catch((error) => {
    throw withPermissionContext(error, 'seller profile record');
  });
  ...
  const publicProfileSnapshot = await getDoc(publicProfileRef).catch((error) => {
    throw withPermissionContext(error, 'public seller location');
  });
  ...
  await setDoc(userRef, {
    ...
    throw withPermissionContext(error, 'seller account activation');
  });
}
...
export async function fetchSubcategoryOptions(categoryName) {
  ...
  const values = Array.isArray(source[normalizedCategory]) ? source[normalizedCategory] : [];
